...
test("inspectLocalReviewStack honors explicit Perplexity and BLACKBOX manager ports", async () => {
  const fetchImpl: typeof fetch = async (input) => {
    ...
    if (url.endsWith(":39233/json/version")) {
      return new Response(JSON.stringify({
        Browser: "Perplexity/2.0"
      }), { status: 200 });
    }
    if (url.endsWith(":38083/health")) {
      return new Response(JSON.stringify({
        status: "ok",
        model_aliases: ["bbxapp/app-agent"],
        state_db_path: "C:/Users/test/AppData/Roaming/BLACKBOXAI/User/globalStorage/state.vscdb",
    ...
    if (url.endsWith(":8045/health")) {
      return new Response(JSON.stringify({ status: "ok", version: "4.1.30" }), { status: 200 });
    }
    if (url.endsWith(":8045/api/proxy/status")) {
      return new Response(JSON.stringify({ running: true, active_accounts: 7 }), { status: 200 });
    }
    if (url.endsWith(":20128/api/version")) {
      return new Response(JSON.stringify({ currentVersion: "0.3.60" }), { status: 200 });
    }
    if (url.endsWith(":20128/api/provider-nodes")) {
      return new Response(JSON.stringify({
        nodes: [
          {
            id: "node-agm",
          ...
          },
          {
            ...
            prefix: "pplxapp",
            apiType: "chat",
            baseUrl: "http://127.0.0.1:30129/v1"
          },
          {
            id: "node-bbx",
            prefix: "bbxapp",
            apiType: "chat",
            baseUrl: "http://127.0.0.1:38083/v1"
          }
        ]
      }), { status: 200 });
    }
    if (url.endsWith(":20128/api/providers")) {
      return new Response(JSON.stringify({
        connections: [
          {
            provider: "node-agm",
  ...
  assert.equal(snapshot.perplexity?.manager_base_url, "http://127.0.0.1:30129");
  assert.equal(snapshot.perplexity?.cdp_base_url, "http://127.0.0.1:39233");
  assert.equal(snapshot.perplexity?.pplxapp_chat.upstream_base_url, "http://127.0.0.1:30129/v1");
  assert.equal(snapshot.blackbox?.manager_base_url, "http://127.0.0.1:38083");
  assert.equal(snapshot.blackbox?.bbxapp_chat.upstream_base_url, "http://127.0.0.1:38083/v1");
});
